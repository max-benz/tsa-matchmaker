...
-- FIXED: height column is INTEGER type, use direct comparison
...
BEGIN
  ...
  WITH fts_results AS (
    ...
    WHERE s.content_tsv @@ plainto_tsquery('english', p_query_text)
      ...
      AND (p_min_height IS NULL OR s.height >= p_min_height)  -- Height filtering (direct comparison)
      AND (p_max_height IS NULL OR s.height <= p_max_height)  -- Height filtering (direct comparison)
  ),
  vector_results AS (
    SELECT
      s.id AS result_id,
