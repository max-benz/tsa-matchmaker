-- Update hybrid_search_singles with CASE-INSENSITIVE state matching
-- Handles: CA, ca, Ca, california, California, CALIFORNIA
...
DECLARE
  p_states_upper TEXT[];
BEGIN
  -- Convert all state filters to uppercase for case-insensitive matching
  IF p_states IS NOT NULL THEN
    p_states_upper := ARRAY(SELECT UPPER(unnest(p_states)));
  END IF;
  ...
  WITH fts_results AS (
    ...
    WHERE s.content_tsv @@ plainto_tsquery('english', p_query_text)
      ...
      -- Case-insensitive state matching: CA, ca, Ca all match
      AND (
        p_states IS NULL
        OR (s.state IS NOT NULL AND UPPER(s.state) = ANY(p_states_upper))
      )
...
-- Test query to check actual state values in your database
-- Run this separately to see what state formats you have
