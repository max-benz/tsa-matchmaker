-- Update hybrid_search_singles function to support multiple states and height range
-- FIXED: Use correct column name 'height' instead of 'height_in_inches'
...
CREATE OR REPLACE FUNCTION hybrid_search_singles(
  ...
  p_states TEXT[] DEFAULT NULL,  -- Changed from p_state to p_states (array)
  p_min_height INT DEFAULT NULL,  -- New: minimum height in inches
  p_max_height INT DEFAULT NULL   -- New: maximum height in inches
)
RETURNS TABLE (
  id INTEGER,
  first_name TEXT,
...
BEGIN
  ...
  WITH fts_results AS (
    ...
    WHERE s.content_tsv @@ plainto_tsquery('english', p_query_text)
      ...
      AND (p_states IS NULL OR s.state = ANY(p_states))  -- Support multiple states
      AND (
        p_min_height IS NULL
        OR (s.height ~ height_numeric_regex AND s.height::INTEGER >= p_min_height)
      )  -- Height filtering (convert text to int)
      AND (
        p_max_height IS NULL
        OR (s.height ~ height_numeric_regex AND s.height::INTEGER <= p_max_height)
      )  -- Height filtering (convert text to int)
  ),
  vector_results AS (
    SELECT
