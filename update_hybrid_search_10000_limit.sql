-- Update hybrid_search_singles function to return entire database by default
...
CREATE OR REPLACE FUNCTION hybrid_search_singles(
  ...
  p_match_count INT DEFAULT 10000, -- Increased from 24 to 10000 to return entire database
...
BEGIN
  ...
  vector_results AS (
    ...
    LIMIT p_match_count * 2  -- Get more vector results to ensure comprehensive coverage
  ),
  combined AS (
