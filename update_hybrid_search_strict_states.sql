-- Update hybrid_search_singles with STRICT state filtering
-- When states are specified, ONLY profiles from those states are included
-- Profiles with NULL state are EXCLUDED when states filter is used
-- This ensures location-based searches are accurate
-- Run this in Supabase SQL Editor

CREATE OR REPLACE FUNCTION hybrid_search_singles(
  p_query_text TEXT,
  p_query_embedding VECTOR(1536),
  p_alpha FLOAT DEFAULT 0.8,  -- Increased from 0.6 to 0.8 to favor exact text matches
...
BEGIN
  ...
  WITH fts_results AS (
    ...
    WHERE s.content_tsv @@ plainto_tsquery('english', p_query_text)
      ...
      -- STRICT state filtering: when states specified, ONLY include those states (no NULLs)
      AND (p_states IS NULL OR (s.state IS NOT NULL AND s.state = ANY(p_states)))
