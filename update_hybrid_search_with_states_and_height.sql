...
BEGIN
  ...
  WITH fts_results AS (
    ...
    WHERE s.content_tsv @@ plainto_tsquery('english', p_query_text)
      ...
      AND (p_min_height IS NULL OR s.height_in_inches >= p_min_height)  -- Height filtering
      AND (p_max_height IS NULL OR s.height_in_inches <= p_max_height)  -- Height filtering
  ),
  vector_results AS (
    SELECT
      s.id AS result_id,
