-- Fix for "column reference id is ambiguous" error
...
-- Test the function
SELECT * FROM hybrid_search_singles(
  p_query_text := 'looking for someone who loves hiking',
  p_query_embedding := (SELECT embedding FROM singles_form_data WHERE embedding IS NOT NULL LIMIT 1),
  p_alpha := 0.6,
  p_match_count := 5
)
LIMIT 3;
